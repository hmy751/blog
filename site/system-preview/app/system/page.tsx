...
import { getExampleArticle, getMarkdownFixtureBody } from "@system/lib/fixtures";
...
export default async function SystemPage() {
  ...
  const fixtureHtml = await markdownToHtml(await getMarkdownFixtureBody(), { leadFirstParagraph: true });

  return (
    <Shell shellClassName={styles.systemShell} footerExtra={<Link href="/system/">System</Link>}>
      <main className="view">
        <SystemHero />
        <ColorSection />
        <TypeSection />
...
function SystemHero() {
  return (
    <section className={styles.sysHero}>
      <div className={styles.sysEyebrow}>Design System · local preview</div>
      <h1 className={styles.sysH1}>디자인 시스템</h1>
      <p className={styles.sysLead}>
        Production App Router에는 등록하지 않지만, 실제 Next components, Markdown pipeline, production styles를 그대로 가져와 확인하는 preview입니다.
      </p>
      <nav className={styles.sysToc} aria-label="Sections">
        ...
        <Link href="/system/example-article/"><span className={styles.num}>View</span> Example Article</Link>
      </nav>
    </section>
  );
}
...
function ProseSection({ fixtureHtml }: Readonly<{ fixtureHtml: string }>) {
  return (
    <SystemSection id="prose" num="03" title="Prose · 본문 요소">
      <p className={styles.sysDesc}>상세 글에서 쓰이는 본문 요소를 production Markdown renderer로 확인한다.</p>
      <Spec label="Inline">
        <div className="prose">
          <p>
...
function ComponentsSection({ posts, example }: Readonly<{ posts: Post[]; example: Post }>) {
  return (
