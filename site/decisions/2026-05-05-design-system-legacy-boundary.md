...
2026-05-05 추가 업데이트: 오해 비용을 줄이기 위해 이 bucket을 `site/archive/design-system/`으로 이동했다. 현재 디자인 시스템 source of truth는 여전히 production code와 Storybook이며, archive 경로는 보존 자료임을 드러내기 위한 위치다.

## 결정

`site/archive/design-system/`은 삭제하지 않고 보존한다. 단, 현재 디자인 판단의 source of truth로 쓰지 않는다.
...
- `archive/design-system/reference/blog-design/`는 Claude Design 원본 evidence archive다.
- `archive/design-system/styles/`는 legacy Node renderer와 과거 이관 과정을 보존하는 reference-derived CSS snapshot이다.
- `archive/design-system/fixtures/`는 Storybook과 system-preview가 아직 공유하는 local-only fixture asset bucket이다. 여기 있는 Markdown과 asset은 QA fixture이며 production content나 current design source가 아니다.

## 적용 규칙

- 새 UI/스타일 작업은 Storybook story와 production CSS/component를 먼저 본다.
- `archive/design-system/reference/blog-design`는 원형 판단을 복원해야 할 때만 evidence로 읽는다.
- `archive/design-system/styles`를 production CSS로 다시 복사하지 않는다. 필요한 경우 현재 `src/styles`와 Storybook에서 재검토한 뒤 고친다.
- `archive/design-system/fixtures`는 local-only QA 자산으로만 쓴다. fixture 문장을 실제 원고나 public route data로 승격하지 않는다.
...
- site 진입 문서가 `archive/design-system`을 active source로 설명하지 않는지 확인한다.
