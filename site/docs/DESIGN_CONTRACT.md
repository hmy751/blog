...
- desktop post table of contents는 본문 measure와 shell/header 중심축을 밀지 않는 오른쪽 overlay rail로 둔다. `.prose h2[id]`, `.prose h3[id]`에서 항목을 만들고 rail 내부 목차만 `position: sticky`로 움직인다. 좁은 화면에서는 목차 DOM을 만들지 않는다.
- global scroll-to-top affordance는 본문 오른쪽 레일에 작게 두고, 기준 스크롤 이하에서는 DOM을 만들지 않는다. mobile에서는 safe-area 기준 우하단에 둔다.
