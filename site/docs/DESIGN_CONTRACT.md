...
1. token/prose CSS는 `site/design-system/styles/index.css`가 전역 source다.
2. `src/content.mjs`가 `content/posts` adapter를 소유한다.
3. `src/render.mjs`가 `Shell`, top nav, home/articles/post/note/about/system DOM contract를 렌더링한다.
4. `src/markdown.mjs`가 Markdown extension transform을 소유한다.
5. Note/About은 site config나 별도 content source가 생기면 실제 데이터에 붙인다.
