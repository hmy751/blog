...
- nav는 현재 `Articles`, `Note`만 노출한다. `About`은 component로 보존하되 route/nav 공개는 나중에 한다.
...
- `Featured`: frontmatter `featured: true`가 있는 글 중 최신순 3개. 없으면 section 자체를 숨긴다.
- `Recent`: 오늘 기준 공개된 최신 글 6개. 명시 featured와 중복되면 제외한다.
...
`dl.about-grid` 패턴을 사용한다. 실제 profile/contact 값은 site config에서 관리한다. 현재는 `src/components/about/AboutPage.tsx`에 보존된 dormant component이며 production route로 등록하지 않는다.
...
현재 `content/posts`는 `description`, `cover`가 거의 없고, `featured`는 사용자가 명시한 글에만 있다. 사이트 adapter가 fallback을 제공하고, 원고를 자동 수정하지 않는다.
...
3. `src/app/**`와 `src/components/**`가 Shell, top nav, home/articles/post/note DOM contract를 렌더링한다. About DOM은 dormant component로 보존한다.
...
6. Note와 dormant About은 site config나 별도 content source가 생기면 실제 데이터에 붙인다.
