...
이 문서는 `../../../blog-design` fixture를 실제 사이트 구현 기준으로 번역한다. 디자인 fixture는 read-only source이고, 구현 기준은 이 문서가 소유한다.

실제 CSS source는 `../design-system/styles/`에 둔다. 앱 scaffold 후에는 `../design-system/styles/index.css`를 전역 stylesheet로 import한다.

## Source Reading

우선순위:

1. `../../../blog-design/Blog v2.html` — live site의 화면/컴포넌트 기준.
2. `../../../blog-design/System.html` — token, typography, prose primitive 기준.
3. `../../../blog-design/screenshots/` — 렌더 느낌 참고. 현재 `home.png`, `home2.png`는 상단부만 남은 stale snapshot으로 보고 HTML보다 낮게 둔다.
4. `../../../blog-design/uploads/draw-*.png` — 피드백/주석 이미지. production asset으로 쓰지 않는다.
5. `../../../blog-design/Blog.html` — v1 prototype. 새 구현 기준으로 쓰지 않는다.
...
1. token/prose CSS를 `site` 앱의 global styles로 옮긴다.
2. `content/posts` adapter를 만든다.
3. `ArticleRow`, `PostMeta`, `PostHero`, `Prose`, `Shell`, `TopNav`, `Footer`를 컴포넌트로 나눈다.
4. home/articles/post를 먼저 실제 Markdown 데이터와 연결한다.
5. Note/About은 site config나 별도 content source가 생기면 붙인다.
6. desktop/mobile screenshot regression을 붙인다.
