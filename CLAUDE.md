...
`~/Desktop/10_work/dev-hub`와 `~/Desktop/10_work/pilab` 자료는 글쓰기의 source/evidence로 읽을 수 있다. 단, 공개 글에는 해석된 문장만 남긴다.

- 내부 경로, 미션 코드 원문, 면접 피드백 원문, 개인 메모 문장을 그대로 옮기지 않는다.
- PI Lab 코드나 파일명/라인 번호가 독자에게 내부 구현을 노출하는 형태로 들어가면 멈춘다.
- sprint 자료는 재료다. 문장을 그대로 붙이지 않고 블로그 문맥으로 다시 쓴다.
...
2. [editorial/README.md](editorial/README.md)를 보고 작업에 맞는 editorial 문서를 먼저 읽는다.
3. 필요한 경우 `blog-source-collector`로 원천 자료를 카드 형태로 모은다.
4. 새 글은 바로 쓰지 않고, 먼저 글의 각도와 구조 후보를 2-3개로 좁힌다.
5. 사용자가 방향을 고른 뒤 v1을 작성한다.
6. v1 이후 `blog-tone-critic`, `blog-structure-critic`, `blog-evidence-checker`를 report-only로 돌린다.
7. 구조 문제가 크면 v2 전 재작성/부분수정 결정을 사용자에게 묻는다.
8. 발행 전에는 `node scripts/blog-prepublish-check.mjs`와 [prepublish-check.md](editorial/prepublish-check.md)를 기준으로 확인한다.

## 목소리와 구조

- [voice.md](editorial/voice.md)는 강제 틀이 아니라 점검 거울이다.
- [edit-patterns.md](editorial/edit-patterns.md)는 반복해서 거부된 수정 방향의 사례집이다.
- [series-pilab.md](editorial/series-pilab.md)는 PI Lab 시리즈에만 우선 적용한다. 다른 글의 전역 구조 법칙으로 쓰지 않는다.
...
- `title`, `date`, `description` frontmatter가 있다.
