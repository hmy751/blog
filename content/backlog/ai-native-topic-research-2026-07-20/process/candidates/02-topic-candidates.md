...
Cofathon에서 Probe 평가 체계와 KRAFTON 가치 정리를 후보 생성의 발산 재료로 사용했다. 서로 다른 두 조건에서 만든 22개 원형을 감사한 뒤, 업무·문제 후보 15개 계열과 별도의 사용자·문제 확보 방식·증명 패턴으로 다시 분류했다. 이 15개 계열은 검증된 제품 후보가 아니라 창의적 가설이었고, 여러 계열이 평가 언어를 업무 문장으로 바꾼 형태였다. 실제 사람, 업무, 망가진 workflow가 입력에 없었기 때문에 후보도 실제 세계에 닻을 내리지 못했다.
...
15개 계열은 제작 후보가 아니라 rubric 대응층으로 내려갔다. 별도의 scene-first 재시작에서는 actor·workflow·입력·결과·확인 수준을 적은 상황 기반 후보 6개를 1차 연구 초안으로 만들었다. 이 6개도 선택·최종 수렴된 후보군은 아니었고, 이후 새 후보 생성의 씨앗에서 제외됐다.
...
1. 좋은 평가 기준을 발산 재료로 넣고 15개 업무·문제 후보 계열이 rubric을 되비추는 형태가 된 장면에서 시작한다.
...
- 15개 업무·문제 후보 계열이 “전부 같았다”는 제목 표현은 사용자 체감의 hook이다. 본문에서는 검증된 제품 후보가 아니었다는 층위, rubric 반향, 실제 장면 부재, 조기 정규화라는 확인 범위를 구체적으로 쓴다.
