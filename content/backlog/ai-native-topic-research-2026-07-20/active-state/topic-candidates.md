...
후보를 평가하는 현재 기준은 [criteria.md](./criteria.md)를 따른다.
...
- 외부 대상만 탓하지 않고 방금 결과를 만든 AI의 결정 위치로 돌아간다.
