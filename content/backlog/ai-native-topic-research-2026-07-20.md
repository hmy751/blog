...
작성일: 2026-07-20
갱신일: 2026-07-23
성격: AI-native 역량 기술 블로그 주제 조사 / 내부 source card
공개상태: 내부 작업 문서
현재상태: 후보군 확정 / 첫 글과 발행 순서는 미정
주요 원천: AX 해커톤, Cofathon, Tripproof, ai-note, 국내 개발 블로그 9편, 2025–2026 공개 1차 자료
---

# AI-native 역량 글감 조사 — 도구가 아니라 개발자의 판단 감각을 고르기

이 조사의 관점은 AI 엔지니어가 LLM을 제품에 적용하는 방법이 아니다. 개발자가 AI와 함께 조사하고, 설계하고, 구현하고, 검증하고, 긴 작업을 이어 갈 때 어떤 판단 감각이 새로 필요한지를 찾는다. LLM judge와 eval은 이 관점을 설명하는 사례가 될 수 있지만, 그 기술 자체가 글의 중심은 아니다.

이 문서는 다섯 가지를 보편적인 AI-native 역량 체계로 확정하는 문서도 아니다. 현재 자료와 사용자의 경험에서 독립된 글로 자랄 만큼 중심 질문과 장면이 있는 후보를 보존하는 조사 문서다.

공개 글로 옮길 때는 이 문서의 로컬 경로, session·task ID, 비공개 대화 원문, 개인 ai-note 문장을 그대로 노출하지 않는다. 수치와 사건은 연결된 원자료에서 다시 확인하고, 공개 가능한 산출물·재현 절차·일반화한 장면으로 바꾼다.
...
### 처음에는 실패 증거가 강한 주제를 먼저 골랐다

초기 조사에서는 초록인 테스트가 놓친 결함, judge를 붙인 뒤 다른 층으로 이동한 오답, self-check 뒤에도 반복된 행동처럼 실패가 분명하고 기술 artifact가 많은 사건을 높게 평가했다. 그 결과 독립 판별을 첫 글로 확정하고, current는 넓은 “문서층위” 주제 아래에 두며, 하네스 과잉과 MCP 전사 누락을 각각 독립 후보로 올렸다.

이 접근은 사실 범위를 지키는 데는 유리했지만 주제 선택 순서를 뒤집었다. 무엇이 실제로 유용했고 어떤 능력을 얻게 됐는지를 먼저 보지 않고, 가장 강하게 증명되는 실패를 먼저 고른 뒤 그 실패에 맞는 역량 이름을 붙였다.

### 국내 개발 블로그 비교에서 가져온 것은 “지속되는 문제와 바뀐 운영”이었다

비교한 글들은 한 번의 팁보다 다음 구조에서 힘이 생겼다.

- 개발자가 바로 알아보는 구체적인 문제나 작업 장면이 있다.
- 한 번의 prompt가 아니라 workflow, 역할, 문서, 테스트, 도구 경계가 바뀐다.
- before와 after 사이에 왜 기존 방식이 부족했는지와 어떤 판단을 거쳤는지가 있다.
- 코드, 설정, 테스트, 측정, 화면 같은 artifact가 주장을 밀어 준다.
- 읽고 나면 특정 도구 이름보다 다른 프로젝트에 옮길 판단 기준이 남는다.

다만 이 패턴을 “모든 글은 실패 → 재설계 → 성공으로 써야 한다”는 템플릿으로 가져오면 안 된다. 회사 규모의 숫자, 도입 성과, 도구 이름, 매끈한 성공 서사도 이번 글의 필수 조건이 아니다. 참고 글의 역할은 표면 구조를 따르는 것이 아니라 개발자가 왜 그 문제를 계속 읽는지 비교하는 데 있다.

### current를 다시 보면서 근본 오류가 드러났다

current의 실제 효용은 “문서를 줄였다”거나 “하네스가 실패했다”가 아니었다. history와 작업 로그가 충분히 있어도 지금 무엇이 유효하고, 전체 작업 중 어디에 있으며, 무엇을 다음에 판단해야 하는지는 자동으로 제어되지 않았다. current는 이때 전체 생애주기, 현재 커서, 현재 효력이 있는 판단, 세부 원천으로 가는 경로를 한 화면에 놓아 긴 context와 재진입 속에서도 작업의 틀을 유지했다.

실패는 이 효용의 반대 증거가 아니라 사용 조건과 한계를 보여 준다. 갱신이 멈추면 stale해지고, 모든 세부를 품으면 다시 비대해지며, hook이 파일을 가리켰다고 모델이 읽고 이해한 것이 증명되지는 않는다.

여기서 주제 선택 순서를 다음처럼 고쳤다.

1. 이전에는 무엇을 할 수 없었는가.
2. 어떤 통제력이나 판단 능력이 새로 생겼는가.
3. 그 효용이 어떤 장면에서 반복됐는가.
4. 무엇이 그 효용을 만들었다고 볼 수 있는가.
5. 근거가 허용하는 주장 범위는 어디까지인가.
6. 실패는 계기, 사용 조건, 한계 중 어디에 놓이는가.

## AI-native 역량으로 보는 범위

여기서 AI-native하게 일한다는 것은 AI를 많이 쓰거나 산출 속도를 높이는 것만을 뜻하지 않는다.

AI는 빠르게 많은 선택지와 결과물을 만들지만, 무엇이 현재 유효한지, 어느 가정을 믿고 있는지, 언제 탐색을 닫아야 하는지, 누가 완료를 선언할 수 있는지, 실패가 어느 층에서 생겼는지를 스스로 안정적으로 소유하지 못한다. 개발자는 결과물을 받는 사람을 넘어 이 불확실한 작업 흐름의 상태·판단·권한·교체 단위를 설계해야 한다.

현재 후보들은 이 공통 문제를 서로 다른 위치에서 다룬다.

- current는 **시간이 길어질 때 상태와 다음 행동을 잃지 않는 문제**를 다룬다.
- 독립 판별은 **구현자와 검증자의 오류가 상관되는 문제**를 다룬다.
- 평가 기준의 시점은 **엄격한 기준이 탐색 자체를 닫아 버리는 문제**를 다룬다.
- self-check는 **AI가 자기 판단을 원인에서 빼는 문제**를 다룬다.
- artifact/core 분리는 **AI가 만든 전체 결과물을 한 덩어리로 채택하거나 폐기하는 문제**를 다룬다.

이 공통성은 시리즈 가능성을 보여 주지만, 다섯 후보를 한 글의 “AI-native 역량 목록”으로 합치지는 않는다. 각 후보는 독립된 긴장과 장면을 가진 별도 글이다.

## 매력적인 글 소재를 고르는 기준

### 1. 새로 생긴 통제력이 있는가

“무엇을 했다”보다 “그 전에는 할 수 없던 무엇을 이제 통제할 수 있게 됐는가”를 본다. 통제력은 현재 상태를 잡는 일, 오류 상관을 끊는 일, 탐색을 열어 둔 채 근거를 엄격히 보는 일, 잘못된 결과물만 선택적으로 버리는 일처럼 실제 다음 행동을 바꿔야 한다.

### 2. 사용자의 경험과 판단이 중심에 있는가

외부 best practice를 요약하는 글보다 사용자가 왜 이 문제에 걸렸고, 어떤 장면에서 효용을 느꼈으며, 처음 판단을 어떻게 바꿨는지가 드러나는 주제가 우선이다. 개인 경험은 보편적 효과 증명은 아니지만 주제를 고르는 일차 자료다.

### 3. 한 번의 팁이 아니라 지속되는 문제인가

prompt 문구 하나나 도구 옵션 하나보다 여러 세션·단계·프로젝트에서 다시 나타난 문제가 강하다. 다만 규모가 크거나 기간이 길어야 한다는 뜻은 아니다. 한 판단이 workflow와 운영 구조를 바꿨는지를 본다.

### 4. 개발자가 붙잡을 구체 장면이 있는가

제목이 추상적이어도 본문에는 코드, 테스트, 상태 전이, 문서 diff, 삭제된 artifact, 실행 결과, 사용자의 정정 같은 장면이 있어야 한다. 장면은 재미를 위한 일화가 아니라 독자가 판단을 복구할 근거다.

### 5. 도구 사용법보다 판단 mechanism이 남는가

current.md, Codex, MCP, LLM judge 같은 구현은 바뀔 수 있다. 글을 읽고 남아야 하는 것은 active state와 history를 분리하는 이유, reviewer의 독립성을 만드는 입력 계약, rubric을 적용할 시점, artifact와 function core를 다른 교체 단위로 보는 기준이다.

### 6. 예상과 실제 사이의 긴장이 있는가

“테스트가 초록이면 제품도 맞다”, “평가 기준을 먼저 주면 더 좋은 아이디어가 나온다”, “기록을 많이 남기면 긴 작업을 이어 갈 수 있다” 같은 자연스러운 예상이 실제 장면에서 어긋날 때 독자는 들어온다. 반전은 필요하지만 과장된 위기나 실패를 억지로 만들지는 않는다.

### 7. 근거가 결론을 고르는 것이 아니라 주장 상한을 정하는가

근거가 가장 많은 사건만 주제로 고르면 사용자의 중요한 경험보다 계측하기 쉬운 실패가 계속 이긴다. 먼저 가치 있는 능력과 질문을 고르고, 근거로 다음을 나눈다.

- 확인된 사실
- 사용자가 반복해서 느낀 효용
- 그 효용을 설명하는 가장 타당한 mechanism
- 아직 증명하지 못한 인과와 일반화

### 8. 내부 맥락을 덜어도 공개 글이 서는가

내부 경로, session 기록, 개인 raw 대화, 해커톤 제출 사정 없이도 독자가 문제와 판단을 이해할 수 있어야 한다. 공개할 수 없는 세부를 제거한 뒤 주장이 무너지면 제목이나 주장 범위를 줄인다.

## 후보를 비교할 때 보는 글의 뼈대

모든 글을 실패 서사에 맞추지 않는다. 후보마다 다음 여섯 질문에 답할 수 있는지 본다.

1. 이전에는 무엇이 제어되지 않았는가.
2. 실제로 새로 생긴 능력이나 효용은 무엇인가.
3. 그 효용이 드러난 대표 장면은 무엇인가.
4. 어떤 구조나 판단이 그 변화를 만들었는가.
5. 다른 개발자가 옮길 수 있는 판단 기준은 무엇인가.
6. 어디까지 말할 수 있고 무엇은 아직 모르는가.
...
## 참고한 국내 개발 블로그에서 가져온 것과 가져오지 않은 것
...
- 알아볼 수 있는 문제에서 시작한다.
- 실제 workflow가 어떻게 달라졌는지 보여 준다.
- 도구 소개보다 선택과 운영의 이유가 남는다.
- before·after를 artifact와 장면으로 확인할 수 있다.
- 한 번의 사건을 넘어 반복해서 쓰이는 구조가 있다.

### 그대로 따르지 않을 것

- 큰 숫자와 조직 규모가 있어야 좋은 주제라는 전제
- 도구 이름을 제목과 논지의 중심으로 두는 방식
- 실패 → 해결 → 성공을 모든 글에 강제하는 구조
- 회사 도입 성과처럼 깔끔하게 닫힌 결말
- 여러 역량을 한 편에서 종합 체계로 소개하는 방식
...
- 글의 관점은 “LLM을 제품에 적용하는 AI engineering”보다 “개발자가 AI와 일하는 판단 감각”이다.
- 주제는 효용과 새로 생긴 통제력에서 고르고, 근거는 주장 범위를 조절한다.
- 실패는 필수 중심이 아니라 계기·조건·한계 중 하나다.
- 사용자에게 반복해서 유용했던 운영 경험은 주제 선택의 일차 자료다.
- 다섯 후보와 각 후보의 중심 질문은 서로 독립적으로 보존한다.
- current는 넓은 문서층위가 아니라 active-state operation을 중심으로 쓴다.
- LLM judge는 현재 독립 판별의 반례로 둔다.
- 하네스 과잉과 MCP 전사 누락은 artifact/core 후보의 배경·후속 장면으로 둔다.

### 열린 것

- 첫 글을 current로 할지 독립 판별로 할지
- 다섯 글을 실제 시리즈로 발행할지 독립 글로 둘지
- 최종 제목과 공개 가능한 대표 장면
- self-check 기존 초안을 얼마나 재작성할지
- artifact/core 글에서 초기 Tripproof meta-stage를 얼마나 포함할지
- 공개 가능한 코드, test 출력, diff, 화면, 도식
- 외부 자료의 최신 내용과 정확한 인용 위치

## 다음 작업

다섯 후보를 다시 넓게 조사하지 않는다. 첫 글을 결정하기 위해 current와 독립 판별 두 후보만 각각 한 장으로 shaping한다.
...
2. 글 전체를 끌 중심 질문
3. 시간순 본 사건
4. 독자가 볼 수 있는 artifact 세 개
5. 사용자의 판단이 바뀐 지점
6. 다른 개발자가 가져갈 기준
7. 마지막에 남길 한계

그 두 장을 비교한 뒤 첫 글과 article type을 결정한다. 이 단계 전에는 전체 초안이나 고정된 시리즈 순서를 만들지 않는다.
