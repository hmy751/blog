...
이 문서는 [../src/](../src/)의 84개 내용을 공식 형식에 끼워 넣는 결정표가 아니다. 같은 내용이 어디에 놓이느냐에 따라 어떻게 작동이 달라지는지 보여주는 경쟁 배치 지도다.
...
첫째, 현재 `src/`의 많은 내용은 core rule 하나로만 구현되지 않는다. 같은 판단을 기억하게 하는 형식과 실제 상황에서 발동시키는 형식, 작동했는지 보는 증거 형식이 별개일 수 있다.
