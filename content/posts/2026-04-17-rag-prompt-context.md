...
## Context를 만드는 세 가지 레버

여기서부터는 프롬프트보다 앞에서 Context를 만드는 레버들을 나눠 봤습니다.
...
표: RAG 검색 레버가 흔들린 지점과 다시 잡은 Context 기준.
...
다만 프롬프트는 사라지는 문제가 아니라, 검색 결과의 역할을 모델에게 알려주는 인터페이스로 남았습니다.
...
마지막에는 이 실험들이 답변 생성보다 앞단의 설계 문제로 모였습니다.
