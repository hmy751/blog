...
비동기 API 요청에 대한 UI상태를 표시하기 위해서는 수동으로 `isLoading`, `error` 같은 상태를 조작해야 했습니다.
...
두 라이브러리 모두 대부분의 메서드나 관리 방식이 비슷한면이 많았습니다.
...
MobX액션을 직접 호출하여 데이터를 가져오고 모든 로딩/에러 상태를 스토어 내에서 수동으로 관리했습니다.
...
캐싱 전략의 부재로 다른 페이지를 방문했다가 다시 돌아올때마다 동일한 API가 중복으로 호출되었지만 SWR을 적용하면서는 데이터를 캐싱하여 동일한 요청은 캐시에서 즉시 반환하도록 했습니다.
...
다시 캐싱 제어를 확인하기 위해 `dedupingInterval` 옵션을 추가로 설정하여 캐싱문제를 해결했습니다.

> 참고: [성능 - SWR](https://swr.vercel.app/ko/docs/advanced/performance)

### 사용자 경험(UX) 개선: 비동기 UI 제공

SWR이 제공하는 `isLoading`, `error` 상태를 활용하면서, 데이터 로딩 중에는 스켈레톤 UI를, 오류 발생 시에는 명확한 에러 메시지를 일관되게 보여줄 수 있었습니다. 이를 통해 화면이 비어있거나 멈춰 보이는 현상을 줄여 사용자가 체감하는 로딩 속도를 개선하고, 초기 콘텐츠 로딩 시간(FCP) 단축에 기여했습니다.
...
- [React에서 서버 데이터를 최신으로 관리하기(React Query, SWR) | 카카오엔터테인먼트 테크블로그](https://tech.kakaoent.com/)
