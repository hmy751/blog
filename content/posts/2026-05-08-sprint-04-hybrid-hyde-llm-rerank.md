...
먼저 검색 실패를 한 덩어리로 보지 않기 위해 정답 후보가 사라지는 위치를 표시했습니다.
...
그 다음 작업은 pool 밖에 있던 정답 후보를 적어도 다음 레이어까지 보내는 것이었습니다.
...
recall이 회복된 뒤에는 병목이 후보 수가 아니라 후보를 다시 고르는 판단으로 옮겨갔습니다.
...
표: Cohere rerank와 LLM rerank를 운영 기준에서 비교한 내용.
...
마지막으로, 같은 실패를 나중에 다시 볼 수 있으려면 개선 결과뿐 아니라 실행 조건도 함께 남아야 했습니다.
