...
이때 검색 개선은 threshold를 낮추는 일로 끝나지 않았습니다. 정답 후보가 어느 레이어에서 사라지는지 보고, 그 레이어를 하나씩 보강해야 했습니다.

이 글에서 따라갈 질문은 하나입니다.

> 정답 chunk는 어느 단계에서 사라졌나?
...
대표로 본 질문 하나가 계속 실패했습니다. 답변은 틀렸고, 처음에는 검색 결과가 약한 것으로 보였습니다.

답변만 보면 실패는 하나처럼 보입니다.

```text
질문 -> 틀린 답변
```

하지만 그 사이에는 정답 chunk가 지나가야 하는 관문이 여러 개 있었습니다.

RAG 검색은 그 사이에서 여러 단계를 거쳤습니다.
...
그래서 처음 한 일은 검색기를 바로 바꾸는 것이 아니라 실패 위치를 표시하는 것이었습니다.
...
vector 검색과 BM25는 서로 다른 단서를 봅니다.

| 검색 경로 | 잘 보는 단서 | 놓치기 쉬운 것 |
| --- | --- | --- |
| vector search | 의미적으로 가까운 표현 | 고유명사, 별칭, 정확한 단어 일치 |
| BM25 | 질문과 문서에 함께 나온 단어 | 표현이 달라진 의미 관계 |
...
HyDE를 붙이는 방식도 조심해야 했습니다. HyDE가 만든 가상 답변은 vector 검색에는 유리하지만, BM25에는 오히려 질문의 날카로운 단어를 희석할 수 있습니다. BM25는 원 쿼리의 고유명사와 키워드가 중요합니다.
...
작은 분리였지만, 여기서 검색 레이어의 역할이 더 선명해졌습니다. 하나의 "개선된 query"를 모든 단계에 밀어 넣는 것이 아니라, 각 검색 레이어가 무엇을 잘하는지에 맞춰 입력을 달리해야 했습니다.

## 후보 수가 아니라 rerank 판단의 문제

HyDE 뒤의 실패는 성격이 달랐습니다.

정답 후보는 이제 보였습니다. 그런데 최종 후보에는 남지 않았습니다.

그래서 다음에는 rerank 후보 수를 늘려 봤습니다. top-3에서 떨어진다면 top-5, top-7로 늘리면 들어올 수 있을 것 같았습니다.
...
이번에는 LLM rerank를 붙였습니다.

여기서 LLM에게 맡긴 역할은 답변 생성이 아니었습니다.

```text
하지 않을 일: 후보를 보고 새 답변을 쓴다
해야 할 일: 후보 목록에서 질문과 관련 있는 index만 고른다
```

후보 목록을 보고 관련 있는 후보의 인덱스만 고르게 했습니다. 생성 모델을 답변자가 아니라 listwise reranker로 쓴 셈입니다.
...
그래서 이 섹션에서 남긴 기준은 LLM rerank를 붙였다는 사실보다, 언제든 Cohere로 되돌릴 수 있게 만든 점에 있었습니다. 강한 모델을 넣는 것은 쉽습니다. 어려운 것은 강한 모델을 쓰는 경로와 비용을 줄이는 경로를 모두 열어두는 일입니다.
...
이 표는 마지막 성공만 보려고 넣은 것이 아닙니다. 중간의 실패들이 각각 다른 정보를 줬다는 점이 더 오래 남았습니다.
...
이 개선을 다시 읽고 반복하려면 검색 알고리즘만으로는 부족했습니다. 관측과 기록이 같이 필요했습니다.
