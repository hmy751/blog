...
점수표를 보기 전에 확인해야 할 것이 생긴 셈입니다.

- 어떤 config로 실행했는가.
- 어떤 prompt version이 붙어 있었는가.
- 결과 파일과 같은 실행을 가리키는 trace가 남아 있는가.
...
그래서 기록을 나중에 쓰는 보고서로만 두면 부족했습니다. 실행이 끝난 뒤에도 모델, 프롬프트, config, trace, 결과를 다시 한 묶음으로 복원할 수 있어야 했습니다.

이 글에서는 그 기준을 실행 계약이라고 부르겠습니다. 거창한 문서를 뜻하려는 말은 아닙니다. 실험 결과를 다시 믿기 위해 어떤 실행 조건을 코드와 기록에 묶었는지, Stage Config, prompt version, snapshot, LangSmith trace 순서로 정리합니다.
...
그런데 결과 파일 하나를 다시 열었을 때 실제로 묻게 되는 질문은 조금 다릅니다.

```text
이 결과는 어느 config에서 나왔나?
그 config는 실행 중에 override됐나?
QA와 Judge prompt는 어느 버전이었나?
같은 요청을 trace에서 다시 찾을 수 있나?
```

RAG 파이프라인에서는 결과가 만들어지기 전까지 너무 많은 조건이 지나갑니다.
...
config에서 먼저 드러난 문제는 "값이 어디 있는가"가 아니었습니다. "이 값이 어느 stage의 책임인가"였습니다.
...
그래서 config를 stage별 책임으로 나눴습니다.

- 전사는 전사 설정을 갖습니다.
- 비전은 비전 설정을 갖습니다.
- correction은 전사 교정 설정을 갖습니다.
- embedding은 chunking과 embedding 설정을 갖습니다.
- retrieval은 검색과 rerank 설정을 갖습니다.
- QA와 Judge는 각자의 모델과 프롬프트 버전을 갖습니다.

겉으로는 config 정리처럼 보이지만, 실제로는 실험에서 "무엇을 바꾸는가"를 코드 구조가 표현하게 만드는 작업에 가까웠습니다.
...
다만 config처럼 숫자나 enum으로 보이지 않을 뿐이었습니다.
...
이렇게 보면 프롬프트 버전 관리는 문장 관리보다 재현성 인프라에 가까웠습니다.

## snapshot이 붙잡아야 하는 것

snapshot은 저장만으로는 부족했습니다. 실행된 값과 기록된 값을 나중에 대조할 수 있어야 했습니다.

실행이 끝난 뒤 아래 둘을 맞춰볼 수 있어야 했습니다.

| 봐야 하는 것 | 틀어지면 생기는 문제 |
| --- | --- |
| 실행에 실제로 쓰인 값 | 결과가 어느 조건에서 나온 것인지 모른다 |
| snapshot에 남은 값 | 기록을 믿고 재실행해도 같은 조건이 아니다 |
...
여기서 더 중요하게 봐야 했던 것은 LangSmith 자체보다 trace의 경계였습니다. 어떤 함수를 trace할지보다, 어떤 실행 단위를 하나의 관측 단위로 볼지가 더 중요했습니다. `qa.request`는 함수 이름이라기보다 질문 한 건을 처리하는 실행 계약에 가까웠습니다.
...
그래서 이 글에서 말하는 기록은 결과를 설명하는 부록에 가깝지 않았습니다. 실행 조건을 고정하고, 나중에 다시 비교할 수 있게 만드는 계약에 더 가까웠습니다.
